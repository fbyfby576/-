...
const rotationAnchorDate = new Date(2026, 5, 1);
...
function renderSelectors() {
  ...
  document.getElementById("dateStatus").textContent = `${formatDate(today)}｜当前显示${currentWeek().name}`;
}
...
function renderDayTabs() {
  document.getElementById("dayTabs").innerHTML = rotatedDayIndexes(shoppingDay)
    .map((index) => {
      const day = currentWeek().days[index];
      return `<button class="day-tab ${index === selectedDay ? "is-active" : ""}" data-day="${index}">${day[0]}<br>${day[1]}</button>`;
    })
    .join("");
}
